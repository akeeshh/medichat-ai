...
if st.session_state.mode == "chat":
    ...
    # Auto-scroll to input field when there are messages (so user doesn't have to scroll after every send)
    if st.session_state.messages:
        ...
        _components.html(
            ...
            <script>
                setTimeout(function() {
                    ...
                    const anchor = iframe.getElementById('chat-input-anchor') ||
                                   iframe.querySelector('[id="chat-input-anchor"]') ||
                                   Array.from(iframe.querySelectorAll('*')).find(el => el.id === 'chat-input-anchor');
                    if (anchor) {
