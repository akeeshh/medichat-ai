...
def medichat_pdf_analysis(question, pdf_text, all_messages, lang_instruction=""):
    ...
    system = (
        ...
        "- Put EVERY heading on its own line, preceded by a blank line. Example:\n"
        "  Greeting paragraph here.\n\n"
        "  ## Heading\n\n"
        "  Content under heading.\n"
        "- Never put a heading inline with paragraph text.\n"
        "- Use ## for section headings (e.g., '## What Stands Out', '## What you should do').\n"
        "- Use **bold** sparingly for key values or emphasis.\n"
        "- Use bullet points (- item) for lists of recommendations or findings, each on its own line.\n"
        "- Leave blank lines between sections so the response is scannable.\n\n"
    )
    if lang_instruction:
...
def markdown_to_html(text):
    """Convert markdown formatting (headings, bold, italics, lists) to clean HTML for rendering inside chat bubbles."""
    if not text:
        return ""
    import re as _re
    import html as _html

    # PRE-NORMALIZE: many LLMs output ## headings or - bullets without leading newlines
    # Force them onto their own lines so the line-by-line parser catches them.
    raw = text
    # Insert blank line BEFORE markdown headings if not already at line start
    raw = _re.sub(r"(?<!\n)\s+(?=#{1,4}\s)", r"\n\n", raw)
    # Insert blank line AFTER heading content (heading line ends at next sentence start, period followed by capital, or end of bold)
    # Detect: ## Heading text [period or capital that starts a new thought]
    raw = _re.sub(r"(#{1,4}\s+[^\n#]+?[\.\:])\s+(?=[A-Z][a-z])", r"\1\n\n", raw)
    # Insert newline BEFORE bullet markers if mid-sentence
    raw = _re.sub(r"(?<!\n)\s+(?=[\-\*]\s+[A-Z])", r"\n", raw)
    # Insert newline BEFORE numbered list markers if mid-sentence
    raw = _re.sub(r"(?<!\n)\s+(?=\d+\.\s+[A-Z])", r"\n", raw)

    # Escape any raw HTML the LLM might have output (safety)
    ...
    for raw_line in lines:
        ...
        if h_match:
            ...
            level = min(level, 4)
            out_lines.append("<h" + str(level + 2) + " class='md-h'>" + content + "</h" + str(level + 2) + ">")
            continue
        ...
        if ul_match:
            if in_ol:
                ...
                in_ol = False
            if not in_ul:
                out_lines.append("<ul class='md-ul'>")
                in_ul = True
            out_lines.append("<li>" + ul_match.group(1) + "</li>")
            continue
        ...
        if ol_match:
            if in_ul:
                ...
                in_ul = False
            if not in_ol:
                out_lines.append("<ol class='md-ol'>")
                in_ol = True
            out_lines.append("<li>" + ol_match.group(1) + "</li>")
            continue

        # Horizontal rule
        if _re.match(r"^[\-\*_]{3,}$", stripped):
            close_lists()
            out_lines.append("<hr class='md-hr'/>")
            continue

        # Regular paragraph
        close_lists()
        out_lines.append("<p class='md-p'>" + stripped + "</p>")
    ...
    html_text = _re.sub(r"(?<!\*)\*([^\*\n]+)\*(?!\*)", r"<em>\1</em>", html_text)
    html_text = _re.sub(r"`([^`\n]+)`", r"<code class='md-code'>\1</code>", html_text)
    ...
    if submit and (user_input.strip() or uploaded_image):
        ...
        if uploaded_image:
                ...
                for event in medichat_rag_stream(user_input, st.session_state.messages, lang_instruction, name_for_rag):
                    ...
                    if kind == "chunk":
                        ...
                        thinking_placeholder.markdown(
                            '<div class="bot-wrap">'
                            '<div class="av av-bot">M</div>'
                            '<div class="bot-bubble">' + markdown_to_html(final_text) + '<span class="stream-cursor"></span></div>'
                            '</div>',
                            unsafe_allow_html=True
                        )
                    elif kind == "done":
            ...
            thinking_placeholder.markdown(
                '<div class="bot-wrap">'
                '<div class="av av-bot">M</div>'
                '<div class="bot-bubble">' + markdown_to_html(final_text) + '</div>'
                '</div>',
                unsafe_allow_html=True
            )
