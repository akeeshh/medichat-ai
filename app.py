...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .bot-bubble .md-p { margin: 0.5rem 0; }
    ...
    .bot-bubble strong { color: var(--sage-900); font-weight: 600; }
    .bot-bubble em { font-style: italic; color: var(--sage-700); }
    .bot-bubble .md-hr { border: none; border-top: 1px solid var(--sage-100); margin: 0.8rem 0; }
    .bot-bubble .md-code {
        background: var(--sage-50);
        color: var(--sage-700);
    ...
    .engine-badge::before { content: "●"; font-size: 0.5rem; }
    .source-tag {
        display: inline-block;
        background: var(--sage-50);
        border: 1px solid var(--sage-100);
    ...
    .input-card {
        ...
        margin-top: 0.6rem;
        box-shadow:
            0 2px 8px rgba(30, 58, 54, 0.04),
            0 12px 40px rgba(30, 58, 54, 0.06);
        transition: border-color 0.25s ease, box-shadow 0.25s ease;
    }
    .input-card:focus-within {
        border-color: var(--sage-300);
        box-shadow:
            0 2px 8px rgba(30, 58, 54, 0.06),
            0 12px 40px rgba(30, 58, 54, 0.10),
            0 0 0 3px rgba(93, 139, 124, 0.10);
    }
    ...
    .input-card .stTextInput > div > div {
        border: none !important;
        box-shadow: none !important;
        background: transparent !important;
    }

    /* Button row inside unified bar */
    .input-actions {
        display: flex;
        align-items: center;
        gap: 0.5rem;
    ...
    .input-attach-wrap [data-testid="stFileUploader"] {
        margin: 0 !important;
    }
    .input-attach-wrap [data-testid="stFileUploader"] > label {
        display: none !important;
    }
    .input-attach-wrap [data-testid="stFileUploader"] section {
        padding: 0 !important;
        background: transparent !important;
        border: none !important;
        min-height: 0 !important;
    }
    .input-attach-wrap [data-testid="stFileUploader"] button {
        background: var(--sage-50) !important;
        color: var(--sage-700) !important;
        border: 1px solid var(--sage-100) !important;
    ...
    .input-attach-wrap [data-testid="stFileUploader"] button:hover {
        background: var(--sage-100) !important;
        border-color: var(--sage-300) !important;
        transform: translateY(-1px) !important;
    }

    /* Send button (primary, right side) */
    .input-send-wrap .stButton > button {
        ...
        border-radius: 12px !important;
        box-shadow: 0 2px 8px rgba(93, 139, 124, 0.25) !important;
        transition: all 0.2s ease !important;
    }
    .input-send-wrap .stButton > button:hover {
        background: linear-gradient(135deg, var(--sage-700), var(--sage-900)) !important;
        box-shadow: 0 4px 14px rgba(93, 139, 124, 0.35) !important;
        transform: translateY(-1px) !important;
    }

    /* Clear button (ghost, right side) */
    .input-clear-wrap .stButton > button {
        background: transparent !important;
    ...
    .input-clear-wrap .stButton > button:hover {
        background: #fdf2f1 !important;
        color: #c4766a !important;
        border-color: #e3bfb8 !important;
    }

    /* Spacer pushes send/clear to the right */
    .input-spacer { flex: 1; }
    ...
    .file-preview-icon.pdf { background: linear-gradient(135deg, #dc2626, #991b1b); }
    .file-preview-icon.img { background: linear-gradient(135deg, var(--sage-500), var(--sage-700)); }
    .file-preview-info { flex: 1; min-width: 0; }
    .file-preview-name { font-weight: 600; font-size: 0.85rem; color: var(--sage-900); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .file-preview-hint { font-size: 0.72rem; color: var(--warm-gray); margin-top: 0.1rem; }
    ...
    .sb-title { font-size: 0.67rem; font-weight: 700; color: var(--soft-gray); text-transform: uppercase; letter-spacing: 0.12em; margin: 0.8rem 0 0.5rem 0; }
    ...
    .input-attach-wrap [data-testid="stAlert"] {
...
if st.session_state.mode == "chat":
    ...
    # ── Action row: [📎 Attach] ····· [Clear] [Send] ──────────────────
    st.markdown('<div class="input-actions">', unsafe_allow_html=True)

    ac1, ac2, ac3, ac4 = st.columns([1.3, 5, 1.2, 1.8])

    with ac1:
        st.markdown('<div class="input-attach-wrap">', unsafe_allow_html=True)
    ...
    with ac3:
        st.markdown('<div class="input-clear-wrap">', unsafe_allow_html=True)
        clear = st.button(L["clear_btn"], use_container_width=True, key="main_clear_btn")
        st.markdown('</div>', unsafe_allow_html=True)

    with ac4:
        st.markdown('<div class="input-send-wrap">', unsafe_allow_html=True)
        submit = st.button(L["send_btn"], use_container_width=True, key="main_send_btn")
        st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('</div>', unsafe_allow_html=True)  # close input-actions

    # ── File preview (shown below unified bar once attached) ───────────
    if uploaded_image:
        is_pdf_upload = uploaded_image.name.lower().endswith(".pdf")
        if is_pdf_upload:
            st.markdown(
                '<div class="file-preview-card">'
                '<div class="file-preview-icon pdf">PDF</div>'
                '<div class="file-preview-info">'
                '<div class="file-preview-name">' + uploaded_image.name + '</div>'
                '<div class="file-preview-hint">Ready for analysis — ask MediChat about this report</div>'
                '</div>'
                '</div>',
                unsafe_allow_html=True
            )
        else:
            st.markdown('<div class="file-preview-card"><div class="file-preview-icon img">IMG</div><div class="file-preview-info"><div class="file-preview-name">' + uploaded_image.name + '</div><div class="file-preview-hint">Image attached — ask a question or just send</div></div></div>', unsafe_allow_html=True)
