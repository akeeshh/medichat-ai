...
def markdown_to_html(text):
    ...
    raw = _re.sub(r"(?<=[.!?])\s+(\*\*[^\*\n]{2,40}:\*\*)", r"\n\n\1\n\n", raw)
    # Insert newline before bullets if mid-line
    raw = _re.sub(r"(?<![\n\-\*])\s+(?=[\-\*]\s+[A-Z])", r"\n", raw)
...
def medichat_rag_stream(question, all_messages, lang_instruction="", patient_name="", pdf_context="", image_context=""):
    ...
    if pdf_context:
        system += (
            ...
            "The patient recently uploaded a medical report. Use this for any follow-up questions they ask about it. "
            "Do NOT re-analyze the entire report unless they explicitly ask. Just answer their specific follow-up question, "
            "referencing the report naturally as if you both already discussed it.\n\n"
            "REPORT CONTENT:\n"
            + pdf_context[:6000]  # cap to avoid token blowup
            + "\n</recent_uploaded_report>\n\n"
        )
    ...
    if image_context:
        system += (
            ...
            "The patient recently uploaded a medical image (skin condition, X-ray, rash, mole, etc.). "
            "Below is your visual analysis from when they first uploaded it. "
            "Use this for any follow-up questions they ask. Do NOT pretend to re-examine the image. "
            "Just answer their follow-up naturally, referencing what was already observed.\n\n"
            + image_context[:3000]
            + "\n</recent_uploaded_image>\n\n"
        )
