...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    /* COMPACT UPLOADER: completely hide native uploader UI, custom button triggers it */
    ...
    /* When a file is uploaded, bring the file-list row back into view */
    .compact-uploader.has-file [data-testid="stFileUploader"] {
    ...
    /* But still hide the dropzone area in has-file mode (only show the file row) */
    .compact-uploader.has-file [data-testid="stFileUploaderDropzone"] {
        display: none !important;
    }
    /* Custom attach button styling */
    .custom-attach-btn {
        display: inline-flex;
        align-items: center;
        gap: 0.5rem;
...
if st.session_state.mode == "chat":
    ...
    # Custom compact attach button. Native uploader is hidden; clicking the custom button
    # finds and clicks the hidden Browse files input via JS.
    has_file_class = " has-file" if False else ""  # state set after file upload below
    st.markdown(
        '<button class="custom-attach-btn" type="button" '
        'onclick="(function(){'
        'const root = window.parent.document;'
        'const input = root.querySelector(\'.compact-uploader input[type=file]\');'
        'if(input) input.click();'
        '})();return false;" '
        'title="Attach a medical image (JPG, JPEG, PNG, up to 200MB) or PDF report">'
        '📎 Attach a medical image or PDF'
        '</button>',
        unsafe_allow_html=True
    )

    # Native Streamlit file_uploader (visually hidden via CSS; only its hidden input is used)
